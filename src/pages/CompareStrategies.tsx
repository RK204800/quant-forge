...
const CompareStrategies = () => {
  ...
  // Build cumulative PnL (dollar) data
  ...
  const correlationMatrix = useMemo(() => {
    ...
    const commonDates = [...allDates].filter((d) => selected.every((s) => returnSeries[s.id].has(d))).sort();
  ...
  // Build total PnL by duration bucket per strategy
  const totalPnlByDurationData = useMemo(() => {
    if (selected.length === 0) return [];
    return DURATION_BUCKETS.map((b, bi) => {
      const row: Record<string, any> = { bucket: b.label };
      selected.forEach((s) => {
        const trades = getTradesInBucket(s.trades, bi);
        row[s.id] = trades.reduce((sum, t) => sum + t.pnlNet, 0);
      });
      return row;
    });
  }, [selected, DURATION_BUCKETS, getTradesInBucket]);
      ...
      {selected.length >= 2 && (
        <div ref={contentRef} className="space-y-6">
          ...
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <div className="flex items-center justify-between">
                <CardTitle className="text-sm font-mono uppercase tracking-wider text-muted-foreground">Cumulative PnL ($)</CardTitle>
                <div className="flex gap-3">
                  {selected.map((s, i) => (
                    <div key={s.id} className="flex items-center gap-1.5">
                      <div className="w-2 h-2 rounded-full" style={{ backgroundColor: COLORS[i % COLORS.length] }} />
            ...
            <CardContent>
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <ComposedChart data={cumulativePnlData}>
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(220 13% 18%)" />
                    <XAxis dataKey="date" tick={{ fill: "hsl(215 15% 55%)", fontSize: 10 }} tickLine={false} axisLine={false} interval="preserveStartEnd" />
                    <YAxis tick={{ fill: "hsl(215 15% 55%)", fontSize: 10 }} tickLine={false} axisLine={false} tickFormatter={(v) => `$${safeNum(v).toLocaleString()}`} />
                    <Tooltip content={<CompareTooltip />} />
                    {selected.map((s, i) => (
                      <Area
                        key={s.id}
          ...
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-mono uppercase tracking-wider text-muted-foreground">Return Correlation Matrix</CardTitle>
            </CardHeader>
            <CardContent>
                  ...
                  <tbody>
                    {selected.map((si, ri) => (
                      <tr key={si.id}>
                        ...
                        {correlationMatrix[ri]?.map((corr, ci) => {
          ...
          {/* Total PnL by Duration */}
          <Card className="bg-card border-border">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm font-mono uppercase tracking-wider text-muted-foreground">Total PnL by Duration ($)</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="h-[300px]">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart data={totalPnlByDurationData} barCategoryGap="20%">
                    <CartesianGrid strokeDasharray="3 3" stroke="hsl(220 13% 18%)" />
                    <XAxis dataKey="bucket" tick={{ fontSize: 10, fill: "hsl(220 9% 46%)" }} />
                    <YAxis tick={{ fontSize: 10, fill: "hsl(220 9% 46%)" }} tickFormatter={(v) => `$${safeNum(v).toFixed(0)}`} />
                    <ReferenceLine y={0} stroke="hsl(220 9% 46%)" strokeDasharray="4 4" strokeOpacity={0.6} />
